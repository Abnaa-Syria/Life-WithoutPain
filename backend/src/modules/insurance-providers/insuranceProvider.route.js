...
router.get('/:id', asyncHandler(async (req, res) => {
  const data = await prisma.insuranceProvider.findUnique({ where: { id: parseInt(req.params.id) } });
...
router.post('/', authenticate, authorize(ROLES.SUPER_ADMIN), asyncHandler(async (req, res) => {
  const data = await prisma.insuranceProvider.create({ data: req.body });
  return createdResponse(res, { data });
}));

router.put('/:id', authenticate, authorize(ROLES.SUPER_ADMIN), asyncHandler(async (req, res) => {
  const data = await prisma.insuranceProvider.update({ where: { id: parseInt(req.params.id) }, data: req.body });
  return successResponse(res, { data });
}));

router.delete('/:id', authenticate, authorize(ROLES.SUPER_ADMIN), asyncHandler(async (req, res) => {
  await prisma.insuranceProvider.delete({ where: { id: parseInt(req.params.id) } });
