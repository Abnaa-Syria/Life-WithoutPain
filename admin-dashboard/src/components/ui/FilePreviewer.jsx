...
const FilePreviewer = ({ files = [], url, title, height = '500px' }) => {
  ...
  const previewFiles = useMemo(() => {
    if (url) {
      return normalizePreviewFiles([{ url, name: title, title }]);
    }
    return normalizePreviewFiles(files);
  }, [files, url, title]);
    ...
    <div className="flex flex-col bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden shadow-sm group">
      <div className="flex items-center justify-between px-4 py-3 bg-[var(--surface-secondary)] border-b border-[var(--border-color)]">
        <div className="flex items-center gap-3 min-w-0">
          <div className="p-2 bg-[var(--surface-secondary)] rounded-lg text-primary-600 shadow-sm shrink-0">
            <FileText size={18} />
          </div>
          <div className="truncate">
            <p className="text-sm font-bold truncate text-[var(--text-primary)]">{fileName}</p>
            <p className="text-[10px] text-[var(--text-muted)]">{currentIndex + 1} / {previewFiles.length}</p>
          </div>
        </div>
      ...
      <div className="relative flex items-center justify-center bg-[var(--surface-secondary)] overflow-hidden" style={{ height }}>
        {previewFiles.length > 1 && (
          <>
            <button
              type="button"
              onClick={handlePrev}
              className="absolute left-2 z-10 p-2 bg-[var(--surface-secondary)]/90 hover:bg-[var(--surface)] text-[var(--text-primary)] rounded-full shadow-lg transition-all opacity-0 group-hover:opacity-100"
            >
              <ChevronLeft size={24} />
            </button>
            <button
              type="button"
              onClick={handleNext}
              className="absolute right-2 z-10 p-2 bg-[var(--surface-secondary)]/90 hover:bg-[var(--surface)] text-[var(--text-primary)] rounded-full shadow-lg transition-all opacity-0 group-hover:opacity-100"
            >
      ...
      {isPdf && numPages && (
        <div className="px-4 py-2 bg-[var(--surface-secondary)] border-t border-[var(--border-color)] flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button
              type="button"
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => setScale((s) => Math.max(0.4, s - 0.1))} className="text-xs font-bold w-6 h-6 flex items-center justify-center hover:bg-[var(--surface-secondary)] rounded">-</button>
            <span className="text-[10px] font-mono font-bold w-10 text-center">{Math.round(scale * 100)}%</span>
            <button type="button" onClick={() => setScale((s) => Math.min(1.5, s + 0.1))} className="text-xs font-bold w-6 h-6 flex items-center justify-center hover:bg-[var(--surface-secondary)] rounded">+</button>
          </div>
        </div>
      )}
    </div>
